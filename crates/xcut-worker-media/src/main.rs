...
fn main() {
    let out = io::stdout();
    let code = std::panic::catch_unwind(main_inner)
        .map(|c| c)
        .unwrap_or_else(|_| {
            let resp = Response {
                protocol: PROTOCOL,
...
fn handle(req: Request) -> Response {
    if req.protocol != PROTOCOL {
        return Response::failure(&req.op, &WorkerError::Protocol(format!(
            "want protocol {PROTOCOL}, got {}",
            req.protocol
        )));
    }
    match req.op.as_str() {
        "describe" => {
            let d = Describe { name: NAME, version: VERSION, protocol: PROTOCOL, ops: OPS };
        ...
        "audio_rms" => {
            let params: AudioRmsParams = if req.params.is_null() {
                ...
                match serde_json::from_value(req.params.clone()) {
                    Ok(p) => p,
                    Err(e) => return Response::failure("audio_rms", &WorkerError::Params(format!("{e}"))),
                }
            };
