...
// same mechanism internal/api and internal/cli use. The canned answer
// travels by environment, never by argument — the product execs the sidecar
// bare, and the stub must survive exactly how the real thing is called;
// children inherit the parent's environment, which is why the installing
// tests use t.Setenv.
...
func runFakeSidecar(mode string) int {
	...
	case req.Op == "analyze":
		...
		if segments == "" {
			segments = `[{"start": 0.5, "end": 1.5, "text": "第一句台词"},{"start": 2.0, "end": 3.0, "text": "第二句台词"}]`
		}
