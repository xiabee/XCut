...
- Linux/macOS 下载对应平台的静态二进制。Linux 侧 FFmpeg 需自行安装
  ...
  已在真机验证全链路（导入→分析→时间线→渲染）通过。该机 4.2.2 构建还
  **不含 `xfade` 滤镜**，所以 `generic_xfade` 风格在它上面渲染不了——换
  `generic_highlight` 或装完整构建（渲染失败信息里就是这么说的）。
...
./xcut auto talk.mp4 --project talk --style beat_shortform --subs=on   # 先补时间线，再转写字幕（按前者的画布排版），最后渲染
...
色块可拖动排序，边缘手柄可裁剪，检查器编辑所选片段的裁剪、速度、音量、运镜与
...
```sh
./xcut subtitles song.mp4 --ass        # 转写 → 样式化 ASS（有词级时间戳则卡拉 OK，否则字幕；默认 SRT）
