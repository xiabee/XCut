...
| 2026-09-24 | **a caption re-lay, measured** (B5e: the tap's and the panel's cheapest caption fix) | `BenchmarkReStyleCaptions` on this host (i7-10875H, Windows, `go test -bench -benchtime=20x`, no `-race`): the product's own path — read `transcript.json`, re-validate it, lay it out for the reel's canvas, `WriteAtomic` the `.ass` — over a 1080×1920 `beat_shortform` reel | 200 cues: **11.4 ms/op** (worst 10.5 ms, 15.0 KiB in → 28.3 KiB out, 0.99 MB allocs). 3000 cues (~2 h of speech): **40.3 ms/op** (worst 51.2 ms, 230.7 KiB in → 416.5 KiB out, 14.9 MB allocs) | against the tap's own measured **9.2 s** end to end (B7a), a re-lay is 0.1–0.6% of what it stands in for; the benchmark asserts `worst <= 2 s` **when it is run** — no gate step runs `-bench`, so this is an on-demand check and not a per-commit ceiling; the numbers above are why the arm is trusted, and re-running `go test -bench 'BenchmarkReStyleCaptions' ./internal/pipeline/` is how they are refreshed | **What is NOT measured here:** the cost of the arm it replaces. A re-transcription runs through the user's own sidecar and model, so no number in this repository covers it and none is invented; the claim being measured is only that a re-lay is milliseconds, which is what makes preferring it over a re-transcription safe to leave in code. The 2 s bound is a headroom assertion, not a target. |
| 2026-09-25 | **GPU render encoder, aligned A/B** (`render.encoder`, M-4) | 20 s of 1280×720 testsrc2 upscaled to the 1920×1080 canvas, 30 fps, one encode each, i7-10875H + RTX 2070, FFmpeg 9.0.2, `xf-probe` @ M-4 worktree | libx264 `-preset veryfast -crf 20`: **2.78 s / 13.82 MB / VMAF 95.97**; h264_nvenc `-rc vbr -b:v 0` at cq 20 / 24 / 28 / 32 → 2.13 / 1.96 / **1.92 s** and 32.2 / 22.0 / **14.5 MB**, VMAF 99.0 / 98.3 / **96.6** / 93.8 | nvenc ≈ **31% faster** at cq 28; `cq = crf + 8` is the measured size/quality parity point, which is why the code ships that offset instead of the raw number | passing CRF raw (`-cq 20`) made **2.3× the bytes** — the scales are not the same and the offset is a measurement, not a convention. Pipeline-level (the 2-clip 20 s probe reel, whole CLI wall): libx264 median 5.5 s vs nvenc 5.3 s over 3 runs each — **parity within noise at this scale**, because per-process NVENC session setup and the fixed decode/filter cost dominate small reels; the balance shifts with reel length and on CPU-bound machines, and rows for real footage remain pending the owner-supplied recording. Doctor reports the posture (`Encoder ✓ h264_nvenc (hardware, render.encoder=auto)`); a machine without the GPU reads the same line as software |
...
| 2026-09-25 | **session #22 (this host): clip-parallel normalization, real footage A/B** | session-0218 reel: 15 clips / 120.0 s output from a 2688×1512 59.94 fps HEVC source, RTX 4090 host (32 threads), `--encoder libx264`, same timeline each run | serial baseline **105.3 s** → ClipWorkers 2: **81.4 s** (1.29×) → ClipWorkers 4 (workspace `max_ffmpeg_processes=4`): **43.4 s (2.43×)**; `h264_nvenc` at 4 workers **43.6 s** — encoder parity once clips parallelize | outputs probed 120.021 s / 18.3 MB (x264) and 19.9 MB (nvenc) | the 2→4 jump only paid off once the *global process limiter* was raised with it: ffprobe (probe) and ffmpeg (normalize) share `max_ffmpeg_processes` slots, so 4 workers on a 2-slot limiter still ran ≈2 effective clips. The fps-first filter reorder (fps resamples before scale/pad) is in every number above — it halves scale work on 59.94→30 without changing kept frames. Wall time now tracks the slowest single clip chain (HEVC decode), which is where NVDEC would be the next lever |
